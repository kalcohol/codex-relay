...
- A 与 B 相互独立、可分别开关；实现为一个 Node 单文件（约 150–250 行）+ 三份端口配置。
...
| 里程碑 | 内容 | 预估 |
|---|---|---|
| M1 | 代理骨架 + 手段 A（含失败安全与空正文保护） | 0.5 天 |
| M2 | 三家验证实验（§6.1） | 0.25 天 |
| M3 | 手段 B（正规 SSE 分帧 + 压缩防御）+ 复验（含本地态/计数断言） | 0.5 天 |
| M4 | 托管形态（常驻服务自重启 + hook/`.ps1` ensure + `/healthz`）+ kill 韧性测试 + 文档 | 0.5 天 |
| M5（可选） | 上游 issue 提交（§9）与跟进 | 0.25 天 |
