...
**计划任务看门狗（保底）+ SessionStart hook（兜底）+ `.ps1` ensure（第二道保险）**，一次安装、长期无感。（2026-09-13 收敛后的最终形态：一个计划任务直接运行单进程 relay，登录时 + 每 1 分钟重复触发充当看门狗，详见 deploy.md §3；方案初稿的"监督进程 + 看护任务"多进程形态已被此取代，演进过程见 deploy.md §6.1。）hook 声明形式（`config/src/hooks_tests.rs` 样例）：
...
09-13 事故（监督进程被外部整体终止 → 孤儿 relay 死后无人接管 → 单端点失守）暴露了"守护进程本身也会死"的结构问题。当日先以"守望模式 + 看护任务"修补，随后进一步收敛：

1. **§4.1 架构**：relay 从"每端点一进程"改为**单进程多端口**（`node relay.js --config relay.config.json`，每端点仍为独立实例、计数独立）；`base_url` 等一切 Codex 侧配置不变；
2. **§5.2 推荐组合**：监督进程层整体移除，改为**计划任务看门狗**——动作直接运行 node（任务实例生死 = 进程生死），登录时 + 每 1 分钟重复触发，实例死了由下一次触发拉起（恢复 ≤60 秒）；看护任务随之取消；
3. **新增崩溃容忍**：uncaughtException / unhandledRejection 记录后继续服务（无状态代理可安全这样做），`/healthz` 以 `recovered_errors` 暴露；日志新增文件落盘（5MB 轮转），因计划任务抓不到 stdout；
4. **取舍**：崩溃恢复从 1–3 秒（监督进程）变为 ≤60 秒（看门狗）——会话中途崩溃该轮可能需重发；换来常驻进程 6 → 1、零守护层。演进过程与事故记录见 deploy.md §6.1。
